...
const getCreators = async (contracts, allAddresses) => {
    ...
    for(const contractAddress of contracts) {
        ...
        if(!allAddresses.includes(response.data.contract_info.creator)) { // Check if creator has already been added
...
const fetchCreators = async (blockHeight, codeIds, allAddresses) => {
    ...
    let spreadsheetData = [["Address", "Stars Balance", "Delegated Stars Balance", "Collection Address", "Launchpad URL"]];
    nonZeroBalanceSnapshot.forEach((addressDetails) => {
        ...
        spreadsheetData.push([
            ...
            `https://app.stargaze.zone/launchpad/${addressDetails.contractAddress}`
        ]);
    });

    const spreadsheetBuffer = xlsx.build([{ name: `Creators Snapshot`, data: spreadsheetData }]);
